...
fn main() {
    ...
    let res_path: Vec<(usize, usize)> = res_path.into_iter().filter(|x| &x.len() / 2 == *part1_res).collect::<Vec<_>>()[0].clone();
    // println!("res_path {:?}", &res_path);
    ...
    for (i, j) in &res_path {
        ...
        if vert_gradient != 0 {
            for col in *j..max_posis.1 {
                ...
                let curr_wind_num = winding_nums.get(&(last_pos.0, col)).expect("At this position is no winding number!");
    ...
    println!("The result of part 2 is {}", &winding_nums.iter().filter(|(_k, &v)| v != 0).collect::<Vec<_>>().len());
}